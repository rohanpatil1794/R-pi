...
def getHistogram(img, minPer=0.1, display=False, region=1):
    ...
    noiseFilter = minPer * maxValue

    indexArray = np.where(histValues >= noiseFilter)
    ...
    if display:
        ...
        for x, intensity in enumerate(histValues):
            ...
            # Ensure circle coordinates are integers
            cv2.circle(
                imgHist,
                (int(basePoint), int(img.shape[0])),
                20,
                (0, 255, 255),
                cv2.FILLED,
            )
